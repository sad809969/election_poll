        ...
        <main className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-[#141E38] border border-slate-800 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white mt-1">22 <span className="text-xs text-emerald-400">(81.5%)</span></h3>
            ...
            <div className="bg-[#141E38] border border-emerald-500/30 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <div>
                <span className="text-xs font-bold text-emerald-400">PDP Statewide Lead</span>
                <h3 className="text-2xl font-extrabold text-white mt-1">+143,665</h3>
                <p className="text-[10px] text-emerald-400 mt-0.5">PDP 562,430 vs APC 418,765</p>
              </div>
              <div className="p-3 rounded-xl bg-emerald-500/20 text-emerald-400"><Award className="w-6 h-6" /></div>
            </div>

            <div className="bg-[#141E38] border border-blue-500/30 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <div>
                <span className="text-xs font-bold text-blue-400">EC8B Ward Forms Verified</span>
                <h3 className="text-2xl font-extrabold text-white mt-1">245 / 287</h3>
                <p className="text-[10px] text-blue-400 mt-0.5">85.3% Ward sheets verified</p>
              </div>
              <div className="p-3 rounded-xl bg-blue-500/20 text-blue-400"><FileText className="w-6 h-6" /></div>
            </div>

            <div className="bg-[#141E38] border border-amber-500/30 rounded-xl p-4 flex items-center justify-between shadow-sm">
              <div>
                <span className="text-xs font-bold text-amber-400">EC8C LGA Sign-offs</span>
                <h3 className="text-2xl font-extrabold text-white mt-1">18 LGAs</h3>
                <p className="text-[10px] text-amber-400 mt-0.5">Signed by returning officers</p>
              </div>
              <div className="p-3 rounded-xl bg-amber-500/20 text-amber-400"><ShieldCheck className="w-6 h-6" /></div>
            </div>
          </div>

          {/* Filter Bar */}
          <div className="bg-[#141E38] border border-slate-800 rounded-xl p-4 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-400">Filter LGA:</span>
              <select 
                value={selectedLga}
          ...
          <div className="bg-[#141E38] border border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-slate-800">
              <h3 className="text-xs font-bold text-slate-200">LGA Collation Progress & Verification Roster</h3>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-900 border-y border-slate-800 text-slate-400 font-bold">
                    ...
                    <th className="py-3 px-3 text-blue-400 font-bold">APC Votes</th>
                    <th className="py-3 px-3 text-purple-400 font-bold">NNPP Votes</th>
                ...
                <tbody className="divide-y divide-slate-800/80 font-medium">
                  {filteredData.map((d, idx) => (
                    <tr key={idx} className="hover:bg-slate-900/50 transition">
                      <td className="py-3 px-3 font-bold text-white text-sm">{d.lga}</td>
                      <td className="py-3 px-3 text-slate-300 font-mono">{d.collatedPus} / {d.totalPus} PUs</td>
                      <td className="py-3 px-3 font-extrabold text-pdp text-sm">{d.pdp.toLocaleString()}</td>
                      <td className="py-3 px-3 font-bold text-blue-400">{d.apc.toLocaleString()}</td>
                      <td className="py-3 px-3 text-purple-400">{d.nnpp.toLocaleString()}</td>
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-white text-xs">{d.pct}%</span>
                      ...
                      <td className="py-3 px-3">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                          d.ec8c === 'Verified' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                      ...
                      <td className="py-3 px-3 text-right">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          d.status === 'Sign-off Ready' ? 'bg-emerald-500/20 text-emerald-400'
                            : d.status === 'Flagged Discrepancy' ? 'bg-red-500/20 text-red-400'
